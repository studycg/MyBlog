...
**Gouraud Shading**：在顶点计算光照，再把颜色插值到像素。**插值的是颜色**。

**Phong Shading**：在像素级**插值法线**，然后在每个像素重新计算光照。

为什么Phong Shading比Gouraud Shading好呢？

Gouraud：**高光**是非常尖锐的函数，可能出现在三角形中间，顶点没有会**直接丢失**。

Phong：不插值结果，插值法线后，每个像素都能自己判断是否产生高光，**高光不会丢失**。
