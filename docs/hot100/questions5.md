...
`while(nums[i]!=[nums[i]-1])`如何理解？

假设有`nums = [3,4,-1,1]`：

第一次while：

`[3,4,-1,1]`：下标0的地方应该存1，但是现在存的是3，那么就把3放再`nums[2]`，此时`nums[2]`位置就好了

`[-1,4,3,1]`：-1不在范围内，次轮结束

第二次while：

`[-1,4,3,1]`：下标1的地方应该放2，但是是4，那就把4放入`nums[3]`，此时`nums[3]`的位置就好了

`[-1,1,3,4]`：下标1的地方应该放2，但是是1，那就把1放入`nums[0]`，此时`nums[0]`的位置就好了
