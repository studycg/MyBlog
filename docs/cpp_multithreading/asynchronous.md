...
**异步编程的核心思想是：**“发出指令后不原地等待结果，而是继续做其他事情，**等结果准备好了再去取。**”
...
这是最底层的工具。你需要手动管理线程的生命周期、处理竞争条件（Mutex）和同步（Semaphore）。

- **优点**：控制力最强。
- **缺点**：开销大，容易出错，难以获取返回值。

### 第二层：任务并行 (`std::async`, `std::future`)

这是中层的工具。你不再关注“线程”，而是关注“任务”。

- **优点**：自动管理生命周期，轻松获取返回值。
- **缺点**：标准库的实现通常没有高效的线程池，频繁创建销毁线程仍有开销。

### 第三层：协程 (C++20 Coroutines)

这是最高级、最现代的异步方式。它允许函数在执行到一半时**挂起（Suspend）**，交出 CPU 控制权，等 IO 或任务完成后再**恢复（Resume）**。

- **核心关键字**：`co_await`, `co_return`, `co_yield`。
- **优势**：可以用“看起来像同步”的代码写出高性能的异步逻辑，极大地减少了内存开销（不需要为每个任务分配完整的线程栈）。